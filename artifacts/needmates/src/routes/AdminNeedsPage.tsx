  ...
  return (
    <AdminShell pageTitle="Needs Management">
      <div className="space-y-5">
        <div className="flex flex-wrap gap-2 text-sm font-bold text-muted-foreground bg-card border-2 border-border rounded-2xl px-5 py-3">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select
            ...
            className="border-2 border-border rounded-xl px-3 py-2 text-sm font-bold focus:outline-none focus:border-primary capitalize"
          ...
          <select
            value={statusFilter}
            onChange={e => setStatusFilter(e.target.value as 'all' | 'active' | 'fulfilled' | 'closed')}
            ...
            <table className="w-full text-sm">
              <thead className="border-b-2 border-border bg-muted/50">
                <tr>
                  ...
                  <th className="text-left font-black px-3 py-3 text-muted-foreground hidden sm:table-cell">Category</th>
                  <th className="text-left font-black px-3 py-3 text-muted-foreground hidden md:table-cell">Posted</th>
                  <th className="text-left font-black px-3 py-3 text-muted-foreground hidden md:table-cell">Interest</th>
                ...
                {filtered.map((need, i) => {
                  ...
                  return (
                    <tr key={need.id} className={cn('border-b border-border last:border-0', i % 2 === 1 && 'bg-muted/20', need.flagged && 'bg-orange-50/50')} data-testid={`need-row-${need.id}`}>
                      ...
                      <td className="px-3 py-4 text-muted-foreground font-semibold hidden md:table-cell">
                      ...
                      <td className="px-3 py-4">
                        <Badge variant={need.status === 'active' ? 'success' : 'outline'} className="capitalize">
                          {need.status}
                        </Badge>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1 flex-wrap">
                          ...
                          {need.status === 'active' && (
                            <Button
                              size="sm"
                              variant="outline"
                              className="rounded-lg border-orange-200 text-orange-600 hover:bg-orange-50 text-xs"
                              onClick={() => closeNeed(need.id)}
                              data-testid={`btn-close-need-${need.id}`}
                            >Close</Button>
                          )}
                          <Button
                            size="sm"
                            variant="outline"
                            className={cn('rounded-lg text-xs', need.flagged ? 'border-orange-300 bg-orange-50 text-orange-600' : 'border-orange-200 text-orange-500 hover:bg-orange-50')}
                            onClick={() => flagNeed(need.id)}
                            data-testid={`btn-flag-need-${need.id}`}
                          >
                          ...
                          {confirmRemove === need.id ? (
                            ...
                            <Button size="sm" variant="outline" className="rounded-lg border-red-200 text-red-500 hover:bg-red-50 text-xs" onClick={() => setConfirmRemove(need.id)} data-testid={`btn-remove-need-${need.id}`}>
                              Remove
                            </Button>
                          )}
                        </div>
